...
const els = {
  date: $('date'), time: $('time'), lat: $('lat'), lon: $('lon'), now: $('now'), load: $('load'),
...
function render(data) {
  ...
  els.details.innerHTML = `<p><b>Paksha:</b> ${escapeHtml(data.paksha_tamil)} / ${escapeHtml(data.paksha)}</p><p><b>Period:</b> ${escapeHtml(data.period_tamil)} / ${escapeHtml(data.period)}</p><p><b>Weekday:</b> ${escapeHtml(data.weekday_tamil)} / ${escapeHtml(data.weekday)}</p><p><b>Samam:</b> ${data.samam}</p><p><b>Tithi:</b> ${escapeHtml(data.tithi?.display || '--')}</p><p><b>Sunrise:</b> ${escapeHtml(data.sunrise || '--')} · <b>Sunset:</b> ${escapeHtml(data.sunset || '--')}</p><p><b>Adhikara:</b> ${escapeHtml(data.adhikara)} · <b>Padu:</b> ${escapeHtml(data.padu)}</p><p><b>Day/night progress:</b> ${Number(data.stats.progress).toFixed(1)}%</p>`;
...
async function calculate() { saveInputs(); els.status.textContent = 'Calculating...'; const params = new URLSearchParams({ date:els.date.value, time:els.time.value, lat:els.lat.value, lon:els.lon.value }); const res = await fetch(`/api/panchapakshi?${params}`); render(await res.json()); }
...
loadInputs(); calculate(); setInterval(() => { setNow(); calculate(); }, 60_000);
